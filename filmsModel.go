...
func (t *films) createFilms(db *sql.DB) (string, error) {
  ...
  statement := fmt.Sprintf("INSERT INTO acme.films(films) VALUES('%s') RETURNING FilmsUUID", jb)
...
// listfilmsFilms: return a list of films
//
...
/*
    qry := `select uuid,
          ...
          from acme.films LIMIT %d OFFSET %d;`
*/
...
// getFilmsFilms: return a films based on the key
//
...
// deletefilmsFilms: return a films based on UID
//
func (t *films) deleteFilms(db *sql.DB, key string) error {
	statement := fmt.Sprintf("DELETE FROM acme.films WHERE FilmsUUID = '%s'", key)
